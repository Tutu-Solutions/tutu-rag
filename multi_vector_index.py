    ...
    def _retrieve(
        ...
        if self._is_embedding_query:
            if query_bundle.embedding is None:
                embedding_from_cache = self.search_from_cache(self._embed_model.model_name, query_bundle.embedding_strs)
                query_bundle.embedding = embedding_from_cache if embedding_from_cache else (
                        self._embed_model.get_agg_embedding_from_queries(
                            query_bundle.embedding_strs
                        )
                    )
                self.insert_to_cache(self._embed_model.model_name, query_bundle.embedding_strs, query_bundle.embedding)
        nodes_with_score = []
...
class MultiVectorStore(BaseIndex[IndexDict]):
    ...
    def as_retriever(self, **kwargs: Any) -> BaseRetriever:
        return MultiVectorIndexRetriever(self._indexes, **kwargs)
